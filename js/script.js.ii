...
// NEW: Function to handle adding Case-Shiller markers
async function addCaseShillerMarkers() {
    // A placeholder for the 20 cities' coordinates
    const cityCoordinates = {
        "Atlanta, GA": [-84.38798, 33.74899],
        "Boston, MA": [-71.0589, 42.3601],
        "Charlotte, NC": [-80.8431, 35.2271],
        "Chicago, IL": [-87.6298, 41.8781],
        "Cleveland, OH": [-81.6944, 41.4995],
        "Dallas, TX": [-96.7970, 32.7767],
        "Denver, CO": [-104.9903, 39.7392],
        "Detroit, MI": [-83.0458, 42.3314],
        "Las Vegas, NV": [-115.1391, 36.1716],
        "Los Angeles, CA": [-118.2437, 34.0522],
        "Miami, FL": [-80.1918, 25.7617],
        "Minneapolis, MN": [-93.2650, 44.9778],
        "New York, NY": [-74.0060, 40.7128],
        "Phoenix, AZ": [-112.0740, 33.4484],
        "Portland, OR": [-122.6750, 45.5231],
    ...
    try {
        // Remove existing Case-Shiller markers before adding new ones
        caseShillerMarkers.forEach(marker => marker.remove());
        caseShillerMarkers = [];

        const response = await fetch('data/ttlc/case_shiller_data.json');
        if (!response.ok) {
            throw new Error(`Failed to load case_shiller_data.json: ${response.statusText}`);
        }
        const data = await response.json();

        data.forEach(cityData => {
            const yoyChange = cityData.yoy_change;
            let markerClass = 'case-shiller-marker';

            if (yoyChange > 0) {
                markerClass += ' case-shiller-positive';
            } else if (yoyChange < 0) {
                markerClass += ' case-shiller-negative';
            } else {
                markerClass += ' case-shiller-zero';
            }

            const el = document.createElement('div');
            el.className = markerClass;
            el.innerHTML = `<span class="case-shiller-label">${yoyChange}%</span>`;

            // Add a popup on click
            const popup = new mapboxgl.Popup({ offset: 25, closeButton: false })
                .setHTML(`
                    <h3>${cityData.city}</h3>
                    <p>YoY Change: <strong>${yoyChange}%</strong></p>
                    <small>S&P CoreLogic Case-Shiller Index</small>
                `);

            const newMarker = new mapboxgl.Marker(el)
                .setLngLat(cityCoordinates[cityData.city])
                .setPopup(popup)
                .addTo(map);

            caseShillerMarkers.push(newMarker);
        });
    } catch (error) {
        console.error(`❌ Error loading Case-Shiller markers:`, error);
    }
}
