...
async function loadRegion(region) {
    ...
    if (metroOverviewMarker) {
        metroOverviewMarker.remove(); // Remove existing marker when loading a new region
                ...
                fetch('data/pinwheels.geojson')
                    ...
                    .then(data => {
                        data.features.forEach(feature => {
                            ...
                            // --- MODIFIED: Calculate average instead of sum for pinwheel tooltip ---
                            //const total = values.reduce((sum, val) => sum + val, 0).toFixed(0); // Old line
...
async function addStaticRegionStats(map) { // Made the function async
    try {
        ...
        // Optionally display a small error message on the map or in the console more prominently
